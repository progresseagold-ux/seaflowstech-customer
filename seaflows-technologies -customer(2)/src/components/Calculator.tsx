...
import { 
  ...
  Download, Send, ArrowRight, Share2, AlertCircle, CheckCircle, Smartphone 
} from 'lucide-react';
...
export default function SolarCctvCalculator({ onSaveQuote, products, currentUser }: CalculatorProps) {
  ...
  const dispatchWhatsApp = (quote: any) => {
    ...
    window.open(`https://wa.me/2348145551234?text=${encoded}`, '_blank');
  };
  ...
  return (
    <div id="calculator-section" className="bg-[#0b1426] text-white p-6 rounded-2xl shadow-2xl border border-gray-800">
      ...
      {/* Tab Selectors */}
      <div id="calc-tabs" className="flex flex-wrap gap-2 mb-8 bg-[#040b15]/60 p-2 rounded-xl border border-gray-800/80">
        <button
          id="tab-sizing"
          onClick={() => setActiveTab('sizing')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
            activeTab === 'sizing' 
              ? 'bg-[#FDB813] text-[#0A2342] shadow-md' 
              : 'text-gray-400 hover:text-white hover:bg-gray-800/40'
          }`}
        >
          <Calculator size={14} /> Solar Sizing Load
        </button>
        <button
          id="tab-solar-quote"
          onClick={() => setActiveTab('solar-quote')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
        ...
        >
          <FileText size={14} /> Solar Instant Quote
        </button>
        <button
          id="tab-cctv-quote"
          onClick={() => setActiveTab('cctv-quote')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
        ...
        >
          <ClipboardList size={14} /> CCTV Camera Quote
        </button>
        <button
          id="tab-installment"
        ...
        >
          <Percent size={14} /> Installments Flex
        </button>
      </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-6">
                ...
                <button
                  onClick={() => {
                    ...
                    window.open(`https://wa.me/2348145551234?text=${encodeURIComponent(text)}`, '_blank');
                  }}
      ...
      {activeTab === 'solar-quote' && (
        <div id="solar-quote-content" className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            <div>
              <h3 className="text-lg font-heading font-bold text-white mb-1">Instant Solar Quotation Engine</h3>
